...
class StripeCheckoutFlowTest(TestCase):
	def setUp(self):
		...
		User = get_user_model()
		# Crear usuario para pruebas locales
		self.user = User.objects.create_user(username='john@gmil.com', email='john@gmil.com', password='123456')

		# Crear categoría y algunos productos
		self.cat = Category.objects.create(name='Electrodomésticos', slug='electrodomesticos')
		self.p1 = Product.objects.create(category=self.cat, name='Producto A', price=100.00, stock=10)
		self.p2 = Product.objects.create(category=self.cat, name='Producto B', price=50.00, stock=5)
	...
	def test_full_checkout_and_webhook_flow(self, mock_construct_event, mock_session_create):
		...
		webhook_resp = self.client.post('/api/sales/stripe-webhook/', data='{}', content_type='application/json', HTTP_STRIPE_SIGNATURE='tst')
